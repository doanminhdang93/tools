...
describe("moneyFormulaForRole — other roles", () => {
  ...
  it("uses the flat 23,000 rate for po", () => {
    expect(moneyFormulaForRole("po", "F", 6)).toBe("=F6*23000");
  });

  it("uses the flat 23,000 rate for designer", () => {
    expect(moneyFormulaForRole("designer", "F", 6)).toBe("=F6*23000");
  });

  it("uses the flat 23,000 rate for marketer", () => {
    expect(moneyFormulaForRole("marketer", "F", 6)).toBe("=F6*23000");
  });
});

describe("poLayoutMoneyFormula", () => {
  it("pays PO points at flat 23,000 and keeps the tester 30% × 45,000 part", () => {
    expect(poLayoutMoneyFormula(6, 7, 20)).toBe(
      '=SUMIF(F7:F20,"PO task",G7:G20)*23000+SUMIF(F7:F20,"Tester task",H7:H20)*0.3*45000',
    );
  });
});
