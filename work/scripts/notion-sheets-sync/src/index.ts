...
import { REVIEW_ELIGIBLE_ROLES, COLUMN_INDEX } from "./constants.ts";
...
async function main(): Promise<void> {
  ...
  for (let memberIndex = 0; memberIndex < ordered.length; memberIndex++) {
    ...
    if (REVIEW_ELIGIBLE_ROLES.has(role) && !pageIdToRowMap) {
      logger.info("Building pageId → row map for review formulas...");
      const builtMap = await buildPageIdToRowMap(members, sheets);
      pageIdToRowMap = builtMap;
      logger.info(`  mapped ${builtMap.size} pages across ${members.length} tabs`);
    }
